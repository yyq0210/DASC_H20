#!/usr/bin/env bash
# RULER suite at 32k/64k/128k context — supplements existing 4k/8k/16k data.
#
# Based on run_ruler_suite_wmax_sweep.sh with long-ctx tuning:
#   - Per-backend mem-fraction-static (GDN NVFP4 needs 0.90, not 0.85)
#   - --max-running-requests 4 (default is too high for 128k prefill → OOM)
#   - PARALLEL=4 (client concurrency matches max-running)
#   - pkill before launch (port-conflict fix from needle test)
#   - Writes to existing LOGDIR so 32k/64k/128k rows merge with 4k/8k/16k
#
# Usage:
#   BACKEND=kda bash experiments/scripts/run_ruler_suite_longctx.sh
#   BACKEND=gdn bash experiments/scripts/run_ruler_suite_longctx.sh
set -uo pipefail
cd "$(dirname "$0")/../.."
...
if [ "$BACKEND" = kda ]; then
  ...
  EXISTING_LOGDIR="$PWD/experiments/results/ruler_suite_kda"
  ...
  MAX_RUNNING="${MAX_RUNNING:-4}"
  EXISTING_LOGDIR="$PWD/experiments/results/ruler_suite_gdn_norecon"
else
...
LENGTHS="${LENGTHS:-32k 64k 128k}"
NUM_SAMPLES="${NUM_SAMPLES:-30}"
NUM_SAMPLES_128K="${NUM_SAMPLES_128K:-20}"
NUM_TRIES="${NUM_TRIES:-4}"
PARALLEL="${PARALLEL:-4}"            # low conc → 128k KV fits
TEMP="${TEMP:-0.0}"
MAXNEW="${MAXNEW:-64}"
if [ "$REPREFILL" = "1" ]; then RECON_TAG=recon; else RECON_TAG=norecon; fi

# Default: merge with existing data (same LOGDIR, same acc_results.jsonl)
LOGDIR="${LOGDIR:-$EXISTING_LOGDIR}"
mkdir -p "$LOGDIR"
RESULT_FILE="$LOGDIR/acc_results.jsonl"
SUMMARY="$LOGDIR/summary_longctx.txt"
# Do NOT truncate — resume-safe (if script restarts, prior summary rows survive)

log(){ echo "[$(date +%H:%M:%S)] $*"; }

echo "[cfg] ruler-suite-longctx backend=$BACKEND model=$(basename "$MODEL") tp=$TP ckpt=$CKPT_SIZE"
...
echo "[cfg]   nsamp=$NUM_SAMPLES (128k=$NUM_SAMPLES_128K) tries=$NUM_TRIES parallel=$PARALLEL"
...
run_server_arm () {   # $1 = arm-tag (dense|w16...), $2 = wmax (0 for dense)
  local TAG="$1" WMAX="$2"
  local SRVLOG="$LOGDIR/server_longctx_${TAG}.log"
  local -a MODE_ENV=()
  if [ "$WMAX" = "0" ]; then
  ...
  # Kill any lingering server on the port BEFORE waiting for GPU
  ...
  for TASK in $RULER_TASKS; do
    for LEN in $LENGTHS; do
      ...
      [ "$LEN" = "128k" ] && NS="$NUM_SAMPLES_128K"
      local ARM="${TAG}_${TASK}_${LEN}"
      # resume: skip if this arm already has a result row.
...
echo ""
echo "===== [ruler-suite-longctx $BACKEND] SUMMARY (recall per task x length x wmax) ====="
...
for length in sorted(by_len.keys()):
    ...
    for arm in sorted(by_len[length]):
        ...
        print(f"{arm+'_'+length:>28} {acc:8.4f} {h:8.3f} {r['num_questions']:5d}")
PY
echo "[done] ruler-suite-longctx $BACKEND -> $RESULT_FILE"
